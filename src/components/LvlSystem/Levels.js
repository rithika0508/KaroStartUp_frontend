...
const Level = (props) => {
  ...
  const [referenceUrl, setReferneceUrl] = useState("");
  const [copyVisibility, setCopyVisibility] = useState(false);

  useEffect(() => {
    if (localStorage.getItem("karostartupUser")) {
      const res = JSON.parse(localStorage.getItem("karostartupUser"));
      props.setVisibility(true);
      setName(res.firstName + " " + res.lastName);
      setLvl(res.level);
      setReferneceUrl(res.referralLink);
    }
  }, [props.visibility]);
  ...
  const getCertificate = async () => {
    const data = await axios.get(
      "http://localhost:5000/api/users/displayCertificate",
      {
        responseType: "arraybuffer",
        headers: {
          Authorization: `Bearer ${JSON.parse(
            localStorage.getItem("karostartupToken")
          )}`,
        },
      }
    );
    await fileDownload(data.data, "karostartup.jpg");
    console.log(data);
  };
        ...
        <div className={styles.main}>
          ...
          <div className={styles.levels__main}>
            ...
            <div className={styles.levelpage}>
              <div className={lvl == 1 ? styles.levelSelected : styles.level}>
                <img
              ...
              <div className={lvl == 2 ? styles.levelSelected : styles.level}>
                <img
                ...
                />
                <h4>Silver Medal</h4>
                <div className={lvl == 3 ? styles.levelSelected : styles.level}>
                  Grab your opportunity to get yours hands on certificate
                  <button onClick={getCertificate} className={styles.download}>
                    <svg
            ...
            <div className={styles.footer}>
              ...
              <div className={styles.connect}>
                <svg
                  ...
                  height="16"
                  fill="currentColor"
                  className={`bi bi-instagram ${styles.connectlogo}`}
                  viewBox="0 0 16 16"
                >
                ...
                <a
                  href="https://www.instagram.com/karo_startup/"
              ...
              <div className={styles.connect}>
                <svg
                  ...
                  height="16"
                  fill="currentColor"
                  className={`bi bi-facebook ${styles.connectlogo}`}
                  viewBox="0 0 16 16"
                >
